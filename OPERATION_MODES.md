...
Важно для пакетной обработки (`max-visits-per-cycle > 1`): после каждого успешного назначения
услуги и перевода визита сервис принудительно обновляет branch cache (`ensureFresh`), чтобы для
следующего визита в этом же цикле использовался актуальный срез очередей и профилей.
Это поведение одинаково для event-trigger и polling-trigger.
...
При обработке нескольких визитов в одном цикле med-robot вызывается последовательно для каждого
визита. После каждого успешного assign+transfer сервис обновляет branch cache, поэтому следующий
вызов робота происходит уже с учетом изменений, внесенных предыдущими визитами в этом же цикле.
Оптимизация: после последнего успешно обработанного визита в цикле дополнительный refresh не делается,
так как в рамках текущего цикла больше нет следующего решения.
