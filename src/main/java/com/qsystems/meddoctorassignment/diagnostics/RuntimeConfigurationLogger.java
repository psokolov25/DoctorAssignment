 ...
 *
 * <p>Нужен для ситуаций, когда архив исходников и реально запущенный артефакт расходятся.
 * На старте пишет ключевые флаги и кодовый источник gateway-реализации, чтобы по логам можно
 * было сразу проверить: подхватились ли нужные настройки и тот ли артефакт реально запущен.
 *
 * <p>В лог сознательно выводятся именно "операционные" признаки: какие trigger-ы активны,
 * как интерпретируется server-side user state, включен ли replay cookie для GET/PUT и какие
 * gateway-реализации реально были подняты в текущем процессе.</p>
 */
@Singleton
public class RuntimeConfigurationLogger implements ApplicationEventListener<StartupEvent> {
    ...
    public RuntimeConfigurationLogger(AssignmentProperties assignmentProperties,
    ...
    public void onApplicationEvent(StartupEvent event) {
        log.info("Runtime configuration marker={} gatewayClass={} gatewayCodeSource={} activationGatewayClass={} activationGatewayCodeSource={} activationEnabled={} activationMethod={} activationPath={} activationFailCycleOnError={} userServicePointSessionStartTriggerEnabled={} workProfileExpandedTriggerEnabled={} setWorkProfileTriggerEnabled={} servicePointOpenTriggerEnabled={} userSessionSettleWindowMs={} abortCycleOnForbiddenMutation={} treatInactiveUserStateAsFailure={} treatNoStartedServicePointSessionAsFailure={} replayMutationCookiesForPut={} replayReadCookiesForGet={} websocketSendCookiesInHandshake={}",
                ...
                assignmentProperties.getActivation() != null && assignmentProperties.getActivation().isEnabled(),
                assignmentProperties.getActivation() != null ? assignmentProperties.getActivation().getMethod() : null,
