...
public interface OrchestraRestClient {
    ...
    @Get("${application.orchestra.common-rest-path}/entrypoint/branches/{branchId}/services/{serviceId}/queue")
    @SingleResult
    Publisher<HttpResponse<TinyQueue>> getQueueForServiceInBranch(int branchId, int serviceId);

    @Get("${application.orchestra.common-rest-path}/managementinformation/v2/branches/{branchId}/servicePoints")
    @SingleResult
    ...
    @Get("${application.orchestra.common-rest-path}/servicepoint/branches/{branchId}/workProfiles/{workProfileId}/queues")
    @SingleResult
    Publisher<HttpResponse<Set<TinyQueue>>> getAllQueuesForWorkProfileInBranch(int branchId, int workProfileId);
