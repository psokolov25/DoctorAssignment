 ...
 * затем детерминированный fallback по {@code serviceId}.
 */
@Singleton
public class DefaultDoctorServiceMatcher implements DoctorServiceMatcher {
  ...
  @Override
  public Optional<SelectedDoctorService> match(
    ...
    for (VisitUnservedService unservedService : visitDetails.getUnservedServices()) {
      ...
      candidates.add(
          new Candidate(
              resolvedServiceId.intValue(),
              targetQueueId.intValue(),
              unservedService.getRouteOrder(),
              priority));
    }
    ...
    candidates.sort(
        Comparator.comparingInt(
                (Candidate candidate) ->
                    candidate.routeOrder != null
                        ? candidate.routeOrder.intValue()
                        : Integer.MAX_VALUE)
    ...
    return Optional.of(
        new SelectedDoctorService(
            selected.serviceId,
            selected.targetQueueId,
            selected.routeOrder,
            selected.describeReason()));
  }
  ...
  private int resolvePriority(VisitUnservedService unservedService, int serviceId) {
    if (unservedService.getExternalKey() != null) {
      return assignmentProperties.resolvePriority(
          unservedService.getExternalKey(), Integer.MAX_VALUE - 1);
    }
